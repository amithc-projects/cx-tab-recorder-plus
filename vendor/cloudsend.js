...
var CloudSend = (() => {
  ...
  __export(index_exports, {
    CloudSendElement: () => CloudSendElement
  });

  // ../sdk/dist/index.js
  var CloudSendError = class extends Error {
  ...
  :host {
    ...
    --cloudsend-bg:        #ffffff;
    --cloudsend-text:      #1a1a2e;
    --cloudsend-muted:     #6b7280;
    --cloudsend-border:    #e5e7eb;
    --cloudsend-accent:    #0052cc;
    --cloudsend-accent-fg: #ffffff;
    --cloudsend-accent-soft: rgba(0, 82, 204, 0.08);
    --cloudsend-row-hover: #f9fafb;
    --cloudsend-row-alt:   #fafafb;
    --cloudsend-chip:      #f3f4f6;
    --cloudsend-chip-hover:#e5e7eb;
    --cloudsend-disabled-bg:#f3f4f6;
    --cloudsend-disabled-fg:#9ca3af;
    --cloudsend-error-bg:  #fee2e2;
    --cloudsend-error-fg:  #991b1b;
    --cloudsend-success-bg:#dcfce7;
    --cloudsend-success-fg:#166534;
    --cloudsend-danger:    #dc2626;

    color: var(--cloudsend-text);
  }

  @media (prefers-color-scheme: dark) {
    :host {
      --cloudsend-bg:        #1a1f3a;
      --cloudsend-text:      #e5e7eb;
      --cloudsend-muted:     #9ca3af;
      --cloudsend-border:    #2d3553;
      --cloudsend-accent:    #5b8def;
      --cloudsend-accent-fg: #0f1729;
      --cloudsend-accent-soft: rgba(91, 141, 239, 0.18);
      --cloudsend-row-hover: #232a4a;
      --cloudsend-row-alt:   #1f2540;
      --cloudsend-chip:      #2d3553;
      --cloudsend-chip-hover:#3a4365;
      --cloudsend-disabled-bg:#2d3553;
      --cloudsend-disabled-fg:#6b7280;
      --cloudsend-error-bg:  #4a1a1a;
      --cloudsend-error-fg:  #fca5a5;
      --cloudsend-success-bg:#064e3b;
      --cloudsend-success-fg:#6ee7b7;
      --cloudsend-danger:    #f87171;
    }
  }

  .root {
    background: var(--cloudsend-bg);
    border: 1px solid var(--cloudsend-border);
    border-radius: 12px;
    overflow: hidden;
    max-width: 560px;
  }

  .section { padding: 1rem 1.25rem; border-bottom: 1px solid var(--cloudsend-border); }
  .section:last-child { border-bottom: none; }

  .label {
    font-size: 0.7rem;
    font-weight: 600;
    color: var(--cloudsend-muted);
  ...
  .target-chip:hover    { background: var(--cloudsend-chip-hover); }
  .target-chip.selected { background: var(--cloudsend-accent); color: var(--cloudsend-accent-fg); }
  ...
  .breadcrumb button {
    background: none;
    border: none;
    color: var(--cloudsend-accent);
    cursor: pointer;
    padding: 0;
    font-size: inherit;
    font-family: inherit;
  }
  .breadcrumb .sep { color: var(--cloudsend-border); }
  .breadcrumb .current { color: var(--cloudsend-text); font-weight: 500; }
  ...
  .browse-row:hover    { background: var(--cloudsend-row-hover); }
  .browse-row.selected { background: var(--cloudsend-accent-soft); }
  .browse-row .arrow,
  .browse-row .leaf-mark { color: var(--cloudsend-muted); font-size: 0.75rem; margin-left: auto; }

  .drop-target { display: flex; flex-direction: column; gap: 0.6rem; }
  .drop-target.drag-over .dropzone,
  .drop-target.drag-over .add-more {
    border-color: var(--cloudsend-accent);
    background: var(--cloudsend-accent-soft);
  }

  .dropzone {
    border: 2px dashed var(--cloudsend-border);
    border-radius: 8px;
    padding: 1.5rem;
    text-align: center;
    cursor: pointer;
  ...
  .dropzone:hover { border-color: var(--cloudsend-accent); background: var(--cloudsend-accent-soft); }
  .dropzone p { margin: 0; font-size: 0.9rem; color: var(--cloudsend-muted); }
  ...
  .add-more {
    ...
    border-radius: 6px;
    border: 1px dashed var(--cloudsend-border);
    background: transparent;
    align-self: flex-start;
    transition: border-color 0.1s, background 0.1s;
  }
  .add-more:hover { border-color: var(--cloudsend-accent); background: var(--cloudsend-accent-soft); }
  ...
  .file-item {
    ...
    background: var(--cloudsend-row-alt);
    border-radius: 6px;
  }
  .file-item .name { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .file-item .size { color: var(--cloudsend-muted); font-size: 0.75rem; }
  .file-item button {
    background: none; border: none; cursor: pointer;
    color: var(--cloudsend-muted); font-size: 1rem; padding: 0 0.25rem;
    line-height: 1; font-family: inherit;
  }
  .file-item button:hover { color: var(--cloudsend-danger); }
  ...
  .btn-primary {
    background: var(--cloudsend-accent);
    color: var(--cloudsend-accent-fg);
  }
  .btn-primary:hover:not(:disabled) { filter: brightness(1.05); }
  .btn-primary:disabled {
    background: var(--cloudsend-disabled-bg);
    color: var(--cloudsend-disabled-fg);
    cursor: not-allowed;
  }
  ...
  .alert.error   { background: var(--cloudsend-error-bg);   color: var(--cloudsend-error-fg); }
  .alert.success { background: var(--cloudsend-success-bg); color: var(--cloudsend-success-fg); }
  ...
  .results { font-size: 0.8rem; color: var(--cloudsend-text); }
  .results summary { cursor: pointer; color: var(--cloudsend-muted); font-size: 0.8rem; }
  .results pre {
    background: var(--cloudsend-row-alt);
    color: var(--cloudsend-text);
    padding: 0.6rem;
  ...
  .progress-label { font-size: 0.78rem; color: var(--cloudsend-muted); margin-bottom: 0.3rem; }
  .progress-bar-track { height: 6px; background: var(--cloudsend-border); border-radius: 999px; overflow: hidden; }
  .progress-bar-fill { height: 100%; background: var(--cloudsend-accent); border-radius: 999px; transition: width 0.15s ease; }
  ...
  .upload-log-entry.ok  { color: var(--cloudsend-success-fg); }
  .upload-log-entry.error { color: var(--cloudsend-error-fg); }
  .log-icon { flex-shrink: 0; font-weight: 700; }
  .log-name { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .log-error { font-size: 0.7rem; color: var(--cloudsend-muted); flex-shrink: 0; max-width: 40%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
`;
  ...
  var CloudSendElement = class extends HTMLElement {
    constructor() {
      super();
      this.client = null;
      this.state = {
    ...
    initClient() {
      ...
      if (endpoint && token) {
        this.client = new CloudSendClient({ endpoint, token });
      }
    }
    setState(patch) {
    ...
    async upload() {
      ...
      for (let i = 0; i < files.length; i += batchSize) {
        ...
        const extraOpts = targetId === "miro" ? { startIndex: i, prevItemId: miroPrevItemId } : void 0;
        let batchResults;
        try {
          const response = await uploadForLeaf(this.client, targetId, leaf, batch, extraOpts);
          batchResults = (response.results || batch.map((f) => ({ filename: f.name, status: "ok" }))).map((r) => ({
            filename: r.filename,
            status: r.status,
            error: r.error || null
          }));
        ...
        failCount += batchResults.filter((e) => e.status !== "ok").length;
        completed += batch.length;
        this.setState({
          uploadProgress: { index: completed, total },
          uploadLog: [...this.state.uploadLog, ...batchResults]
        });
    ...
    errMsg(err) {
      if (err instanceof CloudSendError) return `${err.code}: ${err.message}`;
  ...
  if (typeof customElements !== "undefined" && !customElements.get("zumilabs-cloudsend")) {
    customElements.define("zumilabs-cloudsend", CloudSendElement);
  }

 ...
 */
export function calculateSpreads(pageCount, layoutMode) {
    if (pageCount === 0)
        return [];
    switch (layoutMode) {
        case "single-page":
 ...
 */
export function findSpreadForPage(spreads, page) {
    if (spreads.length === 0)
        return 0;
 ...
 */
export function getSpreadPrimaryPage(spread) {
 ...
 */
export function calculateSpreadLayouts(spreads, pageInfos, scale, pageSpacing, spreadSpacing, dpi, rotation) {
 ...
 */
export function getSpreadDimensions(spread, pageInfos, scale, pageSpacing, dpi, rotation) {
    const dpr = getDevicePixelRatio();
 ...
 */
export function calculateTotalHeight(layouts, spreadSpacing) {
    if (layouts.length === 0)
        return 0;
    const lastLayout = layouts[layouts.length - 1];
    return lastLayout.top + lastLayout.height + snapSpacing(spreadSpacing);
}
function snapSpacing(value) {
    const dpr = getDevicePixelRatio();
 ...
 */
export function findVisibleSpreadRange(layouts, scrollTop, viewportHeight, buffer = 1) {
    if (layouts.length === 0) {
 ...
 */
export function getCroppedPageSize(pageInfo, rotation) {
 ...
 */
export function calculateContinuousLayout(spreads, pageInfos, scale, dpi, rotation, activeGroup) {
...
//# sourceMappingURL=spreadLayout.js.map
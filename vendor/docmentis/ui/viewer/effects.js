...
export function createEffects(store, engine) {
    const unsubscribers = [];
    ...
    unsubscribers.push(store.subscribeEffect(async (prev, next) => {
        if (prev.doc !== next.doc)
            docGeneration++;
        const gen = docGeneration;
        const shouldLoad = next.doc !== null &&
    ...
    unsubscribers.push(store.subscribeEffect(async (prev, next) => {
        ...
        for (const pageIndex of pagesToActuallyLoad) {
            if (gen !== docGeneration)
                return; // document changed, discard
    ...
    unsubscribers.push(store.subscribeEffect(async (prev, next) => {
        ...
        const pagesToLoad = getPagesToLoad(next);
        // Filter to pages that need loading
        const pagesToActuallyLoad = pagesToLoad.filter((pageIndex) => !next.pageText.has(pageIndex) &&
    ...
    unsubscribers.push(store.subscribeEffect((prev, next) => {
        ...
        const queryChanged = prev.searchQuery !== next.searchQuery ||

...
export function reducer(state, action) {
    switch (action.type) {
        ...
        case "NAVIGATE_TO_PAGE": {
            const page = clamp(action.page, 1, Math.max(1, state.pageCount || 1));
            const target = { page };
            if (state.navigationTarget?.page === page &&
        ...
        case "SET_LEFT_PANEL_VISIBLE": {
            ...
            const activePanel = !action.visible && state.activePanel !== null && isLeftPanelTab(state.activePanel)
                ? null
                : state.activePanel;
        ...
        case "SET_ACTIVE_TOOL": {
            ...
            const lastSubToolPerSet = next.kind === "annotate" && state.lastSubToolPerSet.annotate !== next.sub
        ...
        case "SET_SUB_TOOL": {
            ...
            const nextActive = at.kind === "annotate"
                ? { kind: "annotate", sub: action.subTool }
                : { kind: "markup", sub: action.subTool };
            const lastSubToolPerSet = at.kind === "annotate"
                ? { ...state.lastSubToolPerSet, annotate: action.subTool }
                : { ...state.lastSubToolPerSet, markup: action.subTool };
            return {
                ...state,

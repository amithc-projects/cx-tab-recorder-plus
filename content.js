...
if (window.hasZumiLabsOmniCaptRun) return;
...
async function performFullPageCapture(intent) {
  if (window.isZumiLabsOmniCaptCapturing) return;
  window.isZumiLabsOmniCaptCapturing = true;
  try {
  prepareForCapture();
  const preCapture = await applyPreCaptureRules();
  ...
  await downloadImage(stitchedUrl, intent);
    ...
    window.isZumiLabsOmniCaptCapturing = false;
  }
}
...
function createToolbar() {
  ...
  toolbar.innerHTML = `
    <div class="trp-branding">
      ...
      <span class="trp-brand-text">ZumiLabs OmniCapt</span>
    </div>
